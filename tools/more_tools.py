...
def _edit(path: str, old: str = "", new: str = "") -> str:
    ...
    # TODO[Day4] 先实现最稳的 search-replace（old 在文件中唯一时替换为 new）
    #            进阶：支持 unified diff / 整文件重写，比较失败率
    raise NotImplementedError("Day4：实现 edit")
...
def _grep(pattern: str, path: str = ".", max_lines: int = 100) -> str:
    try:
        ...
        return f"[超时] grep 超过 30s 未结束：pattern={pattern} path={path}"

    if p.returncode not in (0, 1):
        error = p.stderr.strip() or f"rg 退出码为 {p.returncode}"
        return f"[grep 出错] {error}"
    ...
    return "\n".join(lines)    
    # TODO[Day4] 调用系统 rg，返回匹配行（带文件名+行号）。与 glob 互补：grep 搜内容，glob 搜路径
    raise NotImplementedError("Day4：实现 grep")
...
def _glob(pattern: str, max_items: int = 100) -> str:
    ...
    return "\n".join(paths) 
    # TODO[Day4] 用 pathlib.Path().glob / rglob 找路径
    raise NotImplementedError("Day4：实现 glob")


# --- web_fetch：URL -> markdown，控 token 预算 ---
def _web_fetch(url: str, max_tokens: int = 2000) -> str:
    # TODO[Day4] httpx 抓取 -> markdownify 转 markdown -> 截断到预算内
