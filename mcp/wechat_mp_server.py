...
def _extract_and_replace_images(md_text: str, uploader: Any) -> str:
...
class WeChatClient:
    ...
    def publish_markdown(
        ...
        # 4. 上传图片
        md_text = _extract_and_replace_images(md_text, self.upload_image)
...
TOOLS = [
    {
        ...
        "description": "【主力工具】读取本地 Markdown 文件 → 上传文中图片 → 创建图文草稿 → 可选直接发布。"
    ...
    {
        "name": "create_draft",
        "description": "用 HTML 正文直接创建图文草稿。如果已有 Markdown 文件，建议用 publish_markdown 更简单。",
        "inputSchema": {
            "type": "object",
            "properties": {
