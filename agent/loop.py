...
from agent.planning import TodoList, with_retry
from agent.output import bind_output_dir, finalize_artifacts, unbind_output_dir
from tools.planning import TODO, bind_todo, unbind_todo
...
class AgentLoop:
    ...
    def run(self, user_task: str, image_paths: list[str] | None = None,
            ...
            history_messages: list[dict[str, Any]] | None = None) -> str:
        ...
        if document_paths:
            ...
            for document_path in document_paths:
                try:
                    ...
                    document_sections.append(
                        f"--- Document {document_path} failed: {exc} ---"
                    )
            user_task = (
                "Reference documents for this task:\n\n"
                + "\n\n".join(document_sections)
                + "\n\n"
                + user_task
            )

        token = bind_todo(self.todo)
        output_token = bind_output_dir(self.output_dir)
        self.todo.items.clear()
        output_path = str(self.output_dir) if self.output_dir else None
        self._emit("run_started", task=user_task, output_dir=output_path)
        try:
            return self._run_bound(
                user_task, image_paths, user_content, history_messages or []
            )
        except Exception as exc:
            self._emit("run_failed", error=str(exc))
            raise
        finally:
            TODO.items = [dict(item) for item in self.todo.items]
            moved = finalize_artifacts(self.workdir)
            if moved:
                self._emit("artifacts_finalized", paths=[str(path) for path in moved])
            unbind_output_dir(output_token)
            unbind_todo(token)
    ...
    def _run_bound(
        ...
        if self.output_dir is not None:
            messages.insert(1, {
                "role": "system",
                "content": (
                    "Run output directory: " + str(self.output_dir) + "\n"
                    "Write event_project.json, activity_plan.md, and WeChat article artifacts "
                    "using their canonical basenames. The file tools route those basenames "
                    "to this run directory. EventProject.outputs must still contain basenames. "
                    "When calling mcp__publish_markdown, pass the actual file path returned "
                    "by the write tool."
                ),
            })
        ...
        for _step in range(1, self.max_turns + 1):
            if self.cancel_event.is_set():
                self._emit("run_failed", error="cancelled", cancelled=True)
                return "Task cancelled"
            if self.todo.items:
                content = TODO_HEADING + self.todo.render()
            ...
            self._emit("assistant_delta", step=_step, content=assistant.get("content", ""))
            ...
            if not tool_calls:
                ...
                self._emit("run_finished", answer=answer)
                return answer

            before = self.todo.snapshot()
            for call in tool_calls:
                if self.cancel_event.is_set():
                    self._emit("run_failed", error="cancelled", cancelled=True)
                    return "Task cancelled"
                ...
                self._emit("tool_started", step=_step, tool=name, arguments=arguments)
                ...
                if tool is None:
                    ...
                    try:
                        ...
                        if verdict == "deny":
                            ...
                            self._emit("approval_required", tool=name, arguments=arguments)
                            approved = bool(
                            ...
                            if approved:
                                ...
                                obs = f"[权限层] 拒绝; 用户未批准：{name}({arguments})"
                ...
                self._emit(
                    "tool_finished", step=_step, tool=name, result=str(obs),
                    duration_ms=round((time.monotonic() - started_at) * 1000),
                )

                if self.verbose:
                    print(f"[result] {truncate_observation(str(obs), 500)}", file=sys.stderr)
                    if name in {"todo_write", "update_todo"} and self.todo.items:
                        print("[todo]\n" + self.todo.render(), file=sys.stderr)

            after = self.todo.snapshot()
            if after != before:
                self._emit("todo_changed", items=[
                    dict(item) for item in self.todo.items
                ])
            if after == before:
                no_progress_steps += 1
            else:
        ...
        answer = self._limit_report("已达步数上限")
        self._emit("run_finished", answer=answer, limited=True)
        return answer

    def _limit_report(self, reason: str) -> str:
        progress = self.todo.render() or "（尚未建立任务清单）"
